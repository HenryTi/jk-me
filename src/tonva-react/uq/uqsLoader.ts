...
export class UQsLoader {
    ...
    constructor(appConfig: AppConfigCore) {
    ...
    */
	...
	private async loadApp():Promise<string[]> {
		let {app, uqs:uqConfigs, /*tvs, */version} = this.appConfig;

		let {name, dev} = app;
        let uqsManApp = new UQsManApp(`${dev.name}/${name}`, undefined/*tvs*/);
	...
	let {uqs:uqConfigs, version/*, tvs*/} = this.appConfig;
		this.uqsMan = new UQsMan(undefined/*tvs*/);
